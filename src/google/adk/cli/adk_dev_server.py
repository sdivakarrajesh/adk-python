...
class AdkDevServer:
  """Core logic for the ADK Development Server endpoints."""
  ...
  def __init__(
    ...
    self.session_trace_dict: dict[str, Any] = {}

    # Set up tracing.
    self.provider = TracerProvider()
    self.provider.add_span_processor(
        export.SimpleSpanProcessor(ApiServerSpanExporter(self.trace_dict))
    )
    self.memory_exporter = InMemoryExporter(self.session_trace_dict)
    self.provider.add_span_processor(
    ...
    self.eval_set_results_manager = LocalEvalSetResultsManager(
    ...
    if memory_service_uri:
      if memory_service_uri.startswith("rag://"):
        ...
        envs.load_dotenv_for_agent("", agents_dir)
        from ..memory.vertex_ai_rag_memory_service import VertexAiRagMemoryService  # pylint: disable=g-import-not-at-top
  ...
  async def close(self):
    """Clean up resources, like closing runners."""
  ...
  def get_trace_dict(self, event_id: str) -> Any:
    event_dict = self.trace_dict.get(event_id, None)
    if event_dict is None:
      raise HTTPException(status_code=404, detail="Trace not found")
    return event_dict

  def get_session_trace(self, session_id: str) -> Any:
    spans = self.memory_exporter.get_finished_spans(session_id)
    if not spans:
      return []
    return [
        {
  ...
  async def create_session(
    ...
    session = await self.session_service.create_session(
        app_name=app_name, user_id=user_id, state=req.state
    )
    if req.events:
  ...
  def _get_eval_set_file_path(self, app_name, eval_set_id) -> str:
    return os.path.join(
        self.agents_dir,
        app_name,
  ...
  async def add_session_to_eval_set(
      self, app_name: str, eval_set_id: str, req: AddSessionToEvalSetRequest
  ):
    # Get the session
    session = await self.session_service.get_session(
  ...
  def delete_eval(self, app_name: str, eval_set_id: str, eval_case_id: str):
    try:
  ...
  async def run_eval(
      self, app_name: str, eval_set_id: str, req: RunEvalRequest
  ) -> list[RunEvalResult]:
  ...
  def list_eval_results(self, app_name: str) -> list[str]:
  ...
  async def list_artifact_versions(
      self, app_name: str, user_id: str, session_id: str, artifact_name: str
  ) -> list[int]:
  ...
  async def agent_run(self, req: AgentRunRequest) -> list[Event]:
    session = await self.session_service.get_session(
        app_name=req.app_name, user_id=req.user_id, session_id=req.session_id
    )
    if not session:
      raise HTTPException(status_code=404, detail="Session not found")
    runner = await self._get_runner_async(req.app_name)
    events = [
        event
  ...
  async def agent_run_sse(self, req: AgentRunRequest) -> StreamingResponse:
    # SSE endpoint
    session = await self.session_service.get_session(
        app_name=req.app_name, user_id=req.user_id, session_id=req.session_id
    )
  ...
  async def agent_live_run(
    ...
    async def forward_events():
      runner = await self._get_runner_async(app_name)
  ...
  async def _get_runner_async(self, app_name: str) -> Runner:

...
from .eval_metrics import MetricNames
...
def _get_default_metric_evaluator_registry() -> MetricEvaluatorRegistry:
  ...
  metric_evaluator_registry.register_evaluator(
      metric_name=MetricNames.TOOL_TRAJECTORY_AVG_SCORE,
      evaluator=type(TrajectoryEvaluator),
  )
  metric_evaluator_registry.register_evaluator(
      metric_name=MetricNames.TOOL_TRAJECTORY_MATCH_V1,
      evaluator=type(TrajectoryEvaluator),
  )
  metric_evaluator_registry.register_evaluator(
      metric_name=MetricNames.RESPONSE_EVALUATION_SCORE,
      evaluator=type(ResponseEvaluator),
  )
  metric_evaluator_registry.register_evaluator(
      metric_name=MetricNames.RESPONSE_MATCH_SCORE,
      evaluator=type(ResponseEvaluator),
  )
  metric_evaluator_registry.register_evaluator(
      metric_name=MetricNames.FINAL_RESPONSE_MATCH_V1,
      evaluator=type(ResponseEvaluator),
  )

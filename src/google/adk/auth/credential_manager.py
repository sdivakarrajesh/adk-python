...
class CredentialManager:
  ...
  Key Features:
  - Pluggable credential exchanger system for different credential types
  - Automatic credential refresh for expired tokens
  - Intelligent caching to avoid redundant processing
  - Support for multiple authentication schemes (OAuth2, OpenID Connect, API keys, etc.)
  - Integration with credential services for persistence

  Workflow:
  1. Validate credential configuration
  2. Check if credential is already ready (no processing needed)
  3. Load existing processed credential from cache or service
  4. If not found, load from authentication response
  5. Exchange credential format if needed (e.g., service account → access token)
  6. Refresh credential if expired
  7. Save processed credential for future use
  ...
  def __init__(
    ...
    self._register_default_refreshers()

  def _register_default_exchangers(self) -> None:
    """Register default credential exchangers."""
  ...
  def _register_default_refreshers(self) -> None:
    """Register default credential refreshers."""
  ...
  def register_credential_refresher(
      self,
      credential_type: AuthCredentialTypes,
  ...
  async def _load_existing_credential(
    ...
    credential = self._load_from_credential_service(tool_context)
    if credential:
      return credential
  ...
  def _load_from_credential_service(
      self, tool_context: ToolContext
  ) -> Optional[AuthCredential]:
    ...
    if credential_service:
      ...
      return credential_service.load_credential(self._auth_config, tool_context)
    return None
  ...
  async def _exchange_credential(
    ...
    exchanged_credential = exchanger.exchange(
  ...
  async def _refresh_credential(
    ...
    if refresher.needs_refresh(credential, self._auth_config.auth_scheme):
      refreshed_credential = refresher.refresh(
          credential, self._auth_config.auth_scheme
      )

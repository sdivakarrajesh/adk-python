...
class InMemoryCredentialService(BaseCredentialService):
  ...
  def _get_bucket_for_current_context(self, tool_context: ToolContext) -> str:
    ...
    if app_name not in self._crendentials:
      self._crendentials[app_name] = {}
    if user_id not in self._crendentials[app_name]:
      self._crendentials[app_name][user_id] = {}
    return self._crendentials[app_name][user_id]

...
class TestOAuth2CredentialRefresher:
  ...
  def test_needs_refresh_token_not_expired(self, mock_oauth2_token):
    ...
    needs_refresh = refresher.needs_refresh(credential, scheme)

    assert not needs_refresh

  @patch("google.adk.auth.refresher.oauth2_credential_refresher.OAuth2Token")
  def test_needs_refresh_token_expired(self, mock_oauth2_token):
  ...
  @patch("google.adk.auth.oauth2_session_helper.OAuth2Session")
  @patch("google.adk.auth.oauth2_session_helper.OAuth2Token")
  def test_refresh_token_expired_success(
    ...
    refresher = OAuth2CredentialRefresher()
    result = refresher.refresh(credential, scheme)
  ...
  def test_needs_refresh_google_oauth2_json_expired(self):
    ...
    with patch(
      ...
      refresher = OAuth2CredentialRefresher()
      needs_refresh = refresher.needs_refresh(credential, None)

      assert needs_refresh

  def test_needs_refresh_google_oauth2_json_not_expired(self):
  ...
  def test_refresh_google_oauth2_json_success(self):
    ...
    with patch(
      ...
      with patch(
        ...
        refresher = OAuth2CredentialRefresher()
        result = refresher.refresh(credential, None)
  ...
  def test_needs_refresh_no_oauth2_credential(self):

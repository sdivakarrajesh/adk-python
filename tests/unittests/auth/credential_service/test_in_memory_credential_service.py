...
class TestInMemoryCredentialService:
  ...
  def test_init(self, credential_service):
    ...
    assert isinstance(credential_service._store, dict)
    assert len(credential_service._store) == 0
  ...
  def test_get_storage_for_current_context_creates_nested_structure(
      self, credential_service, tool_context
  ):
    """Test that _get_storage_for_current_context creates the proper nested structure."""
    storage = credential_service._get_storage_for_current_context(tool_context)

    # Verify the nested structure was created
    assert "test_app" in credential_service._store
    assert "test_user" in credential_service._store["test_app"]
    assert isinstance(storage, dict)
    assert storage is credential_service._store["test_app"]["test_user"]

  def test_get_storage_for_current_context_reuses_existing(
      self, credential_service, tool_context
  ):
    """Test that _get_storage_for_current_context reuses existing structure."""
    # Create initial structure
    storage1 = credential_service._get_storage_for_current_context(tool_context)
    storage1["test_key"] = "test_value"

    # Get storage again
    storage2 = credential_service._get_storage_for_current_context(tool_context)
  ...
  def test_get_storage_different_apps(
    ...
    storage1 = credential_service._get_storage_for_current_context(tool_context)
    storage2 = credential_service._get_storage_for_current_context(
    ...
    assert "test_app" in credential_service._store
    assert "another_app" in credential_service._store
    assert "test_user" in credential_service._store["test_app"]
    assert "another_user" in credential_service._store["another_app"]

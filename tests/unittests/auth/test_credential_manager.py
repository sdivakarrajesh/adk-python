...
class TestCredentialManager:
  ...
  async def test_load_auth_credentials_success(self):
    ...
    result = await manager.load_auth_credential(tool_context)
  ...
  async def test_load_existing_credential_already_exchanged(self):
    ...
    manager = CredentialManager(auth_config)
    manager._load_from_credential_service = Mock(return_value=None)
  ...
  async def test_load_existing_credential_with_credential_service(self):
    ...
    manager = CredentialManager(auth_config)
    manager._load_from_credential_service = Mock(return_value=mock_credential)
  ...
  async def test_load_from_credential_service_with_service(self):
    ...
    credential_service.load_credential.return_value = mock_credential
    ...
    manager = CredentialManager(auth_config)
    result = manager._load_from_credential_service(tool_context)
  ...
  async def test_refresh_credential_oauth2(self):
    ...
    # Mock refresher
    mock_refresher = Mock()
    mock_refresher.needs_refresh.return_value = True
    mock_refresher.refresh.return_value = mock_credential
    ...
    mock_refresher.needs_refresh.assert_called_once_with(
  ...
  async def test_exchange_credentials_service_account(self):
    ...
    with patch.object(
      ...
      mock_exchanger = Mock()
      mock_exchanger.exchange.return_value = mock_exchanged_credential
